...
contactRouter.post("/", async (req, res) => {
  const { name, email, message, company, budgetRange } =
  ...
  try {
    await sendContactEmail({ name, email, message, company, budgetRange });
    console.log(`[contact] Lead ${requestId} — email sent to ${process.env.CONTACT_TO_EMAIL ?? "(not set)"}`);
  } catch (err) {
